...
function batiment_setup() {
	...
	function batiment_change_excerpt( $text )
	{
		$pos = strrpos( $text, '[');
		if ($pos === false)
		{
	...
	add_filter('get_the_excerpt', 'batiment_change_excerpt');



	function batiment_wpex_get_excerpt( $args = array() ) {
	...
	$defaults = array(
		...
		'length'          => 40,
		'readmore'        => false,
		'readmore_text'   => esc_html__( 'read more', 'batiment' ),
		'readmore_after'  => '',
	...
	// Apply filters
	$defaults = apply_filters( 'batiment_wpex_get_excerpt_defaults', $defaults );

	// Parse args
	$args = wp_parse_args( $args, $defaults );

	// Apply filters to args
	$args = apply_filters( 'batiment_wpex_get_excerpt_args', $defaults );
	...
	// No custom excerpt...so lets generate one
		...
		// No more tag defined so generate excerpt using wp_trim_words
			...
			if ( $readmore ) {
				...
				$output .= apply_filters( 'batiment_wpex_readmore_link', $readmore_link );
	...
	return apply_filters( 'batiment_wpex_get_excerpt', $output );
	...
	register_nav_menus( array(
		...
		'menu-3' => esc_html__( 'Left Menu (Left Header Menu for Header Style5)', 'batiment' ),
		'menu-4' => esc_html__( 'Right Menu (Right Header Menu for Header Style5)', 'batiment' ),
